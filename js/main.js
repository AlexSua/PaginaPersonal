window.onload = () => {
    ...
    Array.from(elementos_trabajo).forEach((elemento) => {
        elemento.onclick = () => {
            if (elemento.querySelector(".cronograma-fecha").style.marginTop != "10vh") {
                ...
                cancel_button.onclick = () => {
                    ...
                    elemento.querySelector(".cronograma-contenido-background").classList.remove("open")
                ...
                elemento.querySelector(".title").addEventListener("transitionend", () => {
                    let top = "";
                    if (cronograma_seccion_trabajo.style.width != "50%") {
                        ...
                        elemento.querySelector(".cronograma-contenido").style.display = "flex"
                        elemento.querySelector(".cronograma-contenido-background").classList.add("open")
                       

                    } else {
                        elemento.querySelector(".cronograma-contenido").style.display = "none"
                        Array.from(elementos_trabajo).forEach((el) => {
                            if (el != elemento) {
                                el.style.display = ""
...
function fijarElemento(element, containerCoords) {
    ...
    if (containerCoords.top <= 0 && containerCoords.bottom >= window.innerHeight) {
...
function fijarElementos() {
    ...
    fijarElemento(cancel_button, cronograma_seccion_trabajo_element.getBoundingClientRect())
...
function aparicionElementos() {
    ...
    if (cronograma[0].getBoundingClientRect().top + (window.outerHeight / 3) < window.innerHeight) {
...
function navegacion() {
    var elementos = $(".elemento-cabecera-navegacion");
    elementos[0].onclick = () => {
        navegarA(".info")
    }
    elementos[1].onclick = () => {
